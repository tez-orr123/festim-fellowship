...
H_model.species = [protium, deuterium, tritium]
...
# if there are now three species then I should divide the flux by 3, rightttt?
...
H_model.settings = F.Settings(
    ...
    atol=1e-9,
    rtol=1e-9,
    final_time=3.2e7, # CHANGED TIME TO MAKE IT QUICKER FOR MULTI SPECIES VISUALISATION TESTING
)
...
# three species plot visualisation?????
...
def plot_profile(species, **kwargs):
    ...
    x = problem.t[:] # How would I get time here??
    return plt.plot(x, c, **kwargs)

for species in H_model.species: # Saying problem does not have attribute species
    # change it to H_model object?
