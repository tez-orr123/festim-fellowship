...
trap_density = 1E17 # much higher trapping density than before
...
for vol in my_model.volume_subdomains:
    ...
    u_grid = pyvista.UnstructuredGrid(topology, cell_types, geometry)
    u_grid.point_data["c of H"] = sol.x.array.real
    u_grid.set_active_scalars("c of H")
...
if not pyvista.OFF_SCREEN:
    ...
    figure = u_plotter.screenshot("concentration of H.png")


u_plotter = pyvista.Plotter()

for vol in my_model.volume_subdomains:
    sol = trapped_H.subdomain_to_post_processing_solution[vol]

    topology, cell_types, geometry = plot.vtk_mesh(sol.function_space)
    u_grid = pyvista.UnstructuredGrid(topology, cell_types, geometry)
    u_grid.point_data["c of trapped_H"] = sol.x.array.real
    u_grid.set_active_scalars("c of trapped_H")
    u_plotter.add_mesh(u_grid, cmap="viridis", show_edges=False)
    u_plotter.add_mesh(u_grid, style="wireframe", color="white", opacity=0.2)
...
if not pyvista.OFF_SCREEN:
    ...
    figure = u_plotter.screenshot("concentration of trapped_H.png")
