...
my_model.settings = F.Settings(
    transient=True,
    atol=1e-11, # My tolerances may have been way too high from the discontinuous case before this
    rtol=1e-10,
    final_time=3.2e7, # Works when full year but initial time is less than 10
)
my_model.settings.stepsize = F.Stepsize(
    initial_value=10000, # doesn't like when my initial value isn't 1
    growth_factor=1.1,
    cutback_factor=0.9,
